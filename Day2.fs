...
let validPasswords =
    List.filter (fun ((min, max), letter, password) ->
        let count =
            ...
            |> Array.length
         in min <= count && count <= max) processed

let validPasswords2 = List.filter (fun ((min, max), letter, password:char[]) ->
        let pos1, pos2 = min-1, max-1
        let c1 = password.[pos1] = letter
        let c2 = password.[pos2] = letter
         in (c1 || c2) && not (c1 && c2)) processed


[<EntryPoint>]
let main argv =
    printfn "Valid passwords: %i" validPasswords.Length
    printfn "Valid passwords 2: %i" validPasswords2.Length
    0
